...
export default function Dashboard() {
  ...
  const KpiCard = ({ title, row, higherIsBetter }:{ title:string; row:KpiRow|undefined; higherIsBetter:boolean; }) => {
    const value = row?.value ?? null;
    const unit: Unit = row?.unit ?? "";
    const score = computeScore({ value, unit, higherIsBetter, greenAt: row?.greenAt ?? null, redAt: row?.redAt ?? null });
  ...
  // Live Feed parts (B8,B9,B15,B16,B17) + speed (G12)
  const parts = useMemo(() => {
    const val = (ri: number) => String(rows?.[ri]?.[1] ?? "").trim();
    return { news: val(15), reviews: val(7), social: val(14), bank: val(13), events: val(6) };
  }, [rows]);

  const [pick, setPick] = useState({ news:true, reviews:true, social:true, bank:true, events:true });
  const toggle = (k: keyof typeof pick) => setPick(p => ({ ...p, [k]: !p[k] }));
  const marqueeText = useMemo(() => {
    const order: Array<keyof typeof parts> = ["news","reviews","social","bank","events"];
    return order.filter(k => pick[k]).map(k => parts[k]).filter(Boolean).join("   •   ");
  }, [parts, pick]);
  const marqueeSec = useMemo(() => {
    const raw = rows?.[10]?.[6]; const ctl = Math.max(1, Math.min(100, Number(String(raw ?? "").replace(/[^\d.-]/g, "")) || 70));
  ...
  const chip = (active:boolean): React.CSSProperties => ({
    ...
    border: active ? "1px solid #C9CDF0" : "1px solid #E1E2E6", userSelect:"none"
  });
